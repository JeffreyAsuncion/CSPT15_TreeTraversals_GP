"""
You are given the values from a preorder and an inorder tree traversal. 
Write a
...
def build_tree(preorder, inorder):
    def helper(in_left = 0, in_right = len(inorder)):
        ...
        # set the roots value to the preorder or pre order index
        ...
        # set the roots right to a call of the helper passing in "index + 1" and the "in_right"
        root.right = helper(index + 1, in_right)
        # return root
        return root

    # driver code
    # start from the first per order element
